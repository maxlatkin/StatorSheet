...
public class SheetParams implements ParamsSetting {
	...
	public void setValue(Model currModel) {
		try {
			...
			Parameters.setIntParamValue(ModelParamNames.AA_STATOR_CORE_SLOT_QTY.name(), DataStore.getSlotQty(), currModel);
			Parameters.setDoubleParamValue(ModelParamNames.AA_STATOR_CORE_SHEET_THCK.name(), DataStore.getSheetThck(), currModel);
			Parameters.setIntParamValue(ModelParamNames.AA_STATOR_CORE_SEGM_QTY.name(), DataStore.getSegmQty(), currModel);
